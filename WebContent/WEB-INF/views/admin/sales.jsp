	...
	$.ajax({
		...
		,success:function(searchDateList){
			...
			for(var i=0;i<searchDateList.length;i++){
				...
				var dataDate = parseInt(searchDateList[i]['str_ord_date'].substr(0,4)+searchDateList[i]['str_ord_date'].substr(5,2)+searchDateList[i]['str_ord_date'].substr(8,2)+searchDateList[i]['str_ord_date'].substr(11,2));
				var nowDate = parseInt(year+month+date+hour);
				if(dataDate > nowDate){
					break;
				}
